...
{
  if (_instance == NULL) _instance = new msi_solver( );
  return _instance;
}
msi_solver::msi_solver( ) { field_container = new std::map<pField, int>; }
msi_solver::~msi_solver( )
{
  if (field_container != NULL) field_container->clear( );
...
int msi_solver::get_num_value(pField f)
{
  ...
  if (mit == field_container->end( )) return 0;
  return mit->second;
}
//*******************************************************
...
//*******************************************************
{
  ...
  *end_dof_id_plus_one =
    *start_dof_id +
    num_dof * pumi_mesh_getNumGlobalEnt(pumi::instance( )->mesh, 0);
  return MSI_SUCCESS;
}
//*******************************************************
